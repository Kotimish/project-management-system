...
def load_key(primary_path: Path, fallback_path: Path | None = None) -> str:
    """
    ...
    if primary_path.exists():
        ...
        raise FileNotFoundError(f"Key file not found at: {primary_path}")
    if not key:
        raise ValueError(f"Key file is empty: {primary_path}")
    return key